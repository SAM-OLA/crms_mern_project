...
function Crms700(props) {
  let flagShow = props.flag9
  ...
  const [syndicationname, setsyndicationname] = useState("0");
  const [syndiationtotalamount, setsyndicationtotalamount]= useState(0.00);
...
const fetchAllData = async () => {
  let response = await (
    await fetch(`http://localhost:5000/crms700data?userid=${userid}`)
  ).json();
  setAllData(response);
};
  ...
  const handleSubmit = (event) => {
    ...
    const crms700data = {sno, uniqidentify1,uniidentifno,crmsrefnumber,creditpurpose1,creditpurposesub1,
      feeslist,tenor,benefaccountno,location,relationshiptype,
      ...
      syndicationname,syndiationtotalamount,synrefnumber,collateralpresent,collateralsecure,securitytype,addresssecurity,ownersecurity,
    ...
    console.log(crms700data)
    fetch("http:localhost:5000/crms700data",{
      method:"POST",
      headers:{"content-type":"application/json"},
      body:JSON.stringify(crms700data)
    }).then((res)=>{
  ...
  const handleUpdate = (event) => {
    ...
    fetch(`http://localhost:5000/crms700data?userid=${userid}&id=${searchid}`,{
      method:"PUT",
      headers:{"content-type":"application/json"},
      body:JSON.stringify(crms700data)
    }).then((res)=>{
  ...
  const handleDelete = (event) => {
    ...
    fetch(`http://localhost:5000/crms700data/${searchid}`,{
      method:"DELETE"
    }).then((res)=>{
  ...
  const handleSearch = ()=>{
    let fetchrs = fetch(`http://localhost:5000/crms700data/${searchid}`)
    fetchrs.then(res=>{
      if (res.status >= 400) {
  ...
  return (
    <React.Fragment>
      <h2>CRMS 700 - Update of Migrated Credit Records</h2>
      <div className="myDiv">
...
<div className="row">
  <div className="column">
     ...
     <br></br>
      ...
      <br></br>
      <label htmlFor="Syndiation Total Amount">Syndiation Total Amount :</label>
      <br></br>
      <NumericFormat
      displayType='input'
      thousandSeparator={true}
      decimalSeparator="."
      required
      placeholder='enter syndiationtotalamount'
      name="syndiationtotalamount"
      value={syndiationtotalamount}
      onChange={e =>syndiationtotalamount(e.target.value)}
      >
      </NumericFormat>
      <br></br>
