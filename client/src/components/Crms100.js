...
function Crms100(props) {
  let flagShow = props.flag3
...
const fetchAllData = async () => {
  let response = await (
    await fetch(`http://localhost:5000/crms100data?userid=${userid}`)
  ).json();
  setAllData(response);
};
  ...
  const handleSubmit = (event) => {
    ...
    const crms100data = {sno, uniqidentify1,uniidentifno,legalstatus,credittype,creditpurpose1,creditpurposesub1,creditlimit,outstandingamount,
    ...
    console.log(crms100data)
    fetch("http:localhost:5000/crms100data",{
      method:"POST",
      headers:{"content-type":"application/json"},
      body:JSON.stringify(crms100data)
    }).then((res)=>{
  ...
  const handleUpdate = (event) => {
    ...
    fetch(`http://localhost:5000/crms100data?userid=${userid}&id=${searchid}`,{
      method:"PUT",
      headers:{"content-type":"application/json"},
      body:JSON.stringify(crms100data)
    }).then((res)=>{
  ...
  const handleDelete = (event) => {
    ...
    fetch(`http://localhost:5000/crms100data/${searchid}`,{
      method:"DELETE"
    }).then((res)=>{
  ...
  const handleSearch = ()=>{
    let fetchrs = fetch(`http://localhost:5000/crms100data/${searchid}`)
    fetchrs.then(res=>{
      if (res.status >= 400) {
  ...
  return (
    <React.Fragment>
      <h2>CRMS 100 - Submissions for Federal Govt, State Govts, Local Govts and FCT</h2>
      <div className="myDiv">
